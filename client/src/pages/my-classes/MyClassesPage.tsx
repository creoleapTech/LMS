...
/* ─── Mini Month Stats ─── */
function MiniMonthStats({ monthData }: { monthData: Record<string, { entryCount: number; hasCompleted: boolean; completedCount?: number }> }) {
  const dates = Object.values(monthData);
  const totalClasses = dates.reduce((sum, d) => sum + d.entryCount, 0);
  const completedClasses = dates.reduce((sum, d) => sum + (d.completedCount || 0), 0);
